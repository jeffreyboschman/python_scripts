...
def main(input_dir, output_textfile, subtypes_list, patch_pattern, threshold, verbose):
    ensure_directory_exists(os.path.dirname(output_textfile)) 
    with open(output_textfile, 'w') as output_file:
        sys.stdout = output_file
        all_kappas = list()
        all_auc = list()
        all_average_accs = list()
        ...
        for file_idx, csv_file in enumerate(sorted(glob.glob(os.path.join(input_dir, '*.csv')))):
            ...
            if len(subtypes_list) > 2:
                try:
                    slide_real_labels_one_hot = one_hot(slide_real_labels,len(subtypes_list))
                    print(slide_real_labels_one_hot)
                    slide_pred_labels_one_hot = one_hot(slide_pred_labels,len(subtypes_list))
                    print(slide_pred_labels_one_hot)
                    overall_auc= roc_auc_score(slide_real_labels_one_hot, slide_pred_labels_one_hot, multi_class='ovo', average='macro')
                except ValueError as e:
                    print('Could not calculate AUC, Warning:', e)
            else:
                overall_auc = roc_auc_score(slide_real_labels, slide_pred_labels, average='macro')

#            if len(subtypes_list) == 2:
#                slide_auc = roc_auc_score(slide_real_labels, slide_pred_labels, average='macro')
#            else:
#                slide_auc = roc_auc_score(slide_real_labels, slide_probs_array, multi_class ='ovo', average='macro')
            print_confusion_matrix(conf_matrix, subtypes_list)
            print_results_summary(subtypes_list, acc_per_subtype, weighted_acc, overall_slide_kappa, overall_slide_f1, overall_auc)
